...
import edu.modicon.app.application.dto.profile.UnfollowProfileResponse;
...
@Component
public class UnfollowProfileHandler implements CommandHandler<UnfollowProfileResponse, UnfollowProfileRequest> {
    ...
    @Transactional
    @Override
    public UnfollowProfileResponse handle(UnfollowProfileRequest command) {
        Profile profile = getValidProfile(command.getUsername(), command.getCurrentUsername());
        boolean isFollow = profileRepository.unfollowProfile(profile.getUsername(), command.getCurrentUsername());
